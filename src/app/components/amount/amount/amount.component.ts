...
import {DeleteAmount, getAmount, getAmounts, updateAmount} from '../../../models/amount.model';
...
export class AmountComponent {
  ...
  update(id: number) {
    this.amountService.getAmountById(id).subscribe((amount) => {
      const selectedAmount: getAmount = amount;
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.amountService.pathcAmount(result.value, id).subscribe(
            (response) => {
